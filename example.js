...
async function routes(fastify) {
  ...
  fastify.post("/auth", async (req, reply) => {
    const valid_credentials =
      req.body.user === "John" && req.body.password === "123"; //never use this in production (unsafe). instead, use crypto.timingSafeEqual
    if (!valid_credentials) throw new err.Forbidden();
...
async function privateRoutes(fastify) {
  ...
  fastify.get("/test", async (req, reply) =>
    reply.send({ private: true, message: "Hello, " + req.auth.user + "!" })
  );
}
...
fastify.listen(3000, "0.0.0.0", (err) =>
  console.log(err ? err : "Listening at 3000")
);
