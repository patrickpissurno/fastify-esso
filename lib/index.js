...
module.exports =
  /**
   *
   * @param {import('../index').EssoOptions} opts
   */
  function builder (opts = default_opts) {
    ...
    async function plugin (fastify, options, done) {
      ...
      async function generateAuthToken (data) {
        if (!data || Object.keys(data).length < 1) {
          return 'Bearer ' + (await encrypt(key, '`'))
        } // we need to encrypt something, so let's just save bandwidth
